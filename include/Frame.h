...
public:
    ...
    //[superquadric]
    Eigen::Vector2f ProjectObjCenter(Eigen::Vector3f Center);
...
public:
    ...
    public:
        ...
        float cT;
        line(long unsigned int frameid, string classes, cv::Rect boxes, vector<cv::Point> masks, float confThreshold) :fid(frameid), cla(classes), box(boxes), mask(masks), cT(confThreshold) {}       
    };
    vector<line> yoloLines;
public:

    YOLO(){};
    YOLO(long unsigned int fidVec ,vector<string> clsVec, vector<cv::Rect> bxsVec, vector<vector<cv::Point>>maskpoints, vector<float> ctVec) 
    {

        
            // int currentid;
            // int lastfid;
            cout<<"clsVec.size():"<<clsVec.size()<<endl;
            for (int i = 0; i < clsVec.size(); i++) 
            {
                yoloLines.push_back(line(fidVec, clsVec[i], bxsVec[i], maskpoints[i], ctVec[i]));
            }
    ...
    void printYOLO() 
    {
        for (int i = 0; i < yoloLines.size(); i++) 
        {
            cout << "Line " << i+1 << endl;
            cout << "frameid " << yoloLines[i].fid << endl;
            cout << "Classes: ";

                cout << yoloLines[i].cla << " ";
            
            // cout << endl << "Boxes: ";

            //     cout << "(" << yoloLines[i].box.x << "," << yoloLines[i].box.y << "," << yoloLines[i].box.width << "," << yoloLines[i].box.height << ") ";
            
            // cout << endl << "Masks: ";

            //     for (cv::Point& point : yoloLines[i].mask) 
            //     {
            //             std::cout << "Point (" << point.x << ", " << point.y << ")" << std::endl;
                    
            //     }

            //     // cout << "x="<< yoloLines[i].mask.x << " "<< "y="<< yoloLines[i].mask.y<< " ";
    
            cout << endl << "Confidence Threshold: "  << endl;

                cout << yoloLines[i].cT << " "<< endl;
