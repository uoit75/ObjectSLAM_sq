...
void Optimizer::ObjectOptimize(Map *pMap, Object* MapObjects) 
{
    ...
    for(int i = 0; i < MapObjects->Objects.size(); i++)
    {
        ...
        //Calculate the maximum distance for setting vertex weights.
        ...
        for (int j = 0; j < MapObjects->Objects[i].Obj_MPs.size(); j++) 
        {
            ...
            edge->setId(j);
            edge->setVertex(0, v);             
            float weight = (MP_pos - Obj_center).norm() / Max_distance;//Eq.11 in paper.

            //Construct a Huber robust kernel function.
            g2o::RobustKernelHuber* robust_kernel_huber = new g2o::RobustKernelHuber;
            robust_kernel_huber->setDelta(0.5);
            edge->setRobustKernel(robust_kernel_huber);

            edge->setInformation(Eigen::Matrix<double, 1, 1>::Identity() * weight); // Assign weights to each point.
            optimizer.addEdge(edge);
        }

        // Perform optimization, iterating 30 times.
        optimizer.initializeOptimization();
...
//Optimize the parameters of the hyperquadric shape function, it is not utilized in the actual code.
...
//A function for outputting optimization analysis information, it is not utilized in the actual code.
            ...
            for(int i = 1; i < Markov_length; i++)
            {
                ...
                float Energy_new = 0; 
                    ...
                    for (int j = 0; j < MapObjects->Objects[i].Obj_MPs.size(); j++) 
                    {
                        ...
                        edge->setInformation(Eigen::Matrix<double, 1, 1>::Identity() * weight); 
                        optimizer.addEdge(edge);
                    }
