...
void LoadImagesReplica(const string &strImagePath, const string &strPathTimes,
...
int main(int argc, char **argv)
{
    ...
    for (seq = 0; seq<num_seq; seq++)
    {
        ...
        LoadImagesReplica(string(argv[3*(seq+1)]), string(argv[3*(seq+1)+1]), vstrImageFilenames[seq], vTimestampsCam[seq]);
        cout << "LOADED!" << endl;
    ...
    for (seq = 0; seq<num_seq; seq++)
    {
        ...
        for(int ni=0; ni<nImages[seq]; ni++, proccIm++)
        {
            ...
            if(im.empty())
            {
                cerr << endl << "Failed to load empty image at: "
                ...
                vector<string> &vstrImages, vector<double> &vTimeStamps)
{
    ...
    while(!fTimes.eof())
    {
        ...
        if(!s.empty())
        {
            ...
            int pos = s.find(',');
            int end = s.find(".");
            string item = s.substr(pos+1, end - pos - 1);
            string timedata = s.substr(0, pos);

            vstrImages.push_back(strImagePath + "/" + item + ".jpg");

            double t = stod(timedata);
