...
void EdgeInertial::linearizeOplus()
{
    ...
    _jacobianOplus[0].block<3,3>(0,0) = -invJr*Rwb2.transpose()*Rwb1; // 15-75
    // (3,0)起点的3*3块表示速度残差对pose1的旋转求导
    _jacobianOplus[0].block<3,3>(3,0) = Sophus::SO3d::hat(Rbw1*(VV2->estimate() - VV1->estimate() - g*dt)); // 15-90
    // (6,0)起点的3*3块表示位置残差对pose1的旋转求导
    _jacobianOplus[0].block<3,3>(6,0) = Sophus::SO3d::hat(Rbw1*(VP2->estimate().twb - VP1->estimate().twb
                                                   - VV1->estimate()*dt - 0.5*g*dt*dt)); // 15-102
    // translation
    // (6,3)起点的3*3块表示位置残差对pose1的位置求导
    _jacobianOplus[0].block<3,3>(6,3) = -Eigen::Matrix3d::Identity(); // 15-98

    // Jacobians wrt Velocity 1
    // _jacobianOplus[1] 9*3矩阵 总体来说就是三个残差分别对pose1的速度求导
    _jacobianOplus[1].setZero(); // 15-73
    _jacobianOplus[1].block<3,3>(3,0) = -Rbw1; // 15-85
    _jacobianOplus[1].block<3,3>(6,0) = -Rbw1*dt; // 15-100
    ...
    _jacobianOplus[2].block<3,3>(6,0) = -JPg; // 15-93

    // Jacobians wrt Accelerometer 1
    // _jacobianOplus[3] 9*3矩阵 总体来说就是三个残差分别对加速度计偏置的速度求导
    _jacobianOplus[3].setZero(); // 15-73
    _jacobianOplus[3].block<3,3>(3,0) = -JVa; // 15-83
    _jacobianOplus[3].block<3,3>(6,0) = -JPa; // 15-94

    // Jacobians wrt Pose 2
    // _jacobianOplus[4] 9*6矩阵 总体来说就是三个残差分别对pose2的旋转与平移（p）求导
    _jacobianOplus[4].setZero(); //15-81
    // rotation
    _jacobianOplus[4].block<3,3>(0,0) = invJr; // 15-77
    // translation
    _jacobianOplus[4].block<3,3>(6,3) = Rbw1*Rwb2; // 15-96

    // Jacobians wrt Velocity 2
    // _jacobianOplus[5] 9*3矩阵 总体来说就是三个残差分别对pose2的速度求导
    _jacobianOplus[5].setZero();
    _jacobianOplus[5].block<3,3>(3,0) = Rbw1; // 15-88
}
