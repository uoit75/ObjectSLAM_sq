...
// void YOLOv8Seg::preprocess(const std::vector<cv::Mat>& imgsBatch)
// {
//     std::cout << "Entering preprocess function." << std::endl;
//     std::cout << "Batch size: " << m_param.batch_size << std::endl;
//     std::cout << "Source width: " << m_param.src_w << ", Source height: " << m_param.src_h << std::endl;
//     std::cout << "Destination width: " << m_param.dst_w << ", Destination height: " << m_param.dst_h << std::endl;

//     std::cout << "Calling resizeDevice..." << std::endl;
//     resizeDevice(m_param.batch_size, m_input_src_device, m_param.src_w, m_param.src_h,
//         m_input_resize_device, m_param.dst_w, m_param.dst_h, 114, m_dst2src);
//     std::cout << "resizeDevice completed." << std::endl;

//     std::cout << "Calling bgr2rgbDevice..." << std::endl;
//     bgr2rgbDevice(m_param.batch_size, m_input_resize_device, m_param.dst_w, m_param.dst_h,
//         m_input_rgb_device, m_param.dst_w, m_param.dst_h);
//     std::cout << "bgr2rgbDevice completed." << std::endl;

//     std::cout << "Calling normDevice..." << std::endl;
//     normDevice(m_param.batch_size, m_input_rgb_device, m_param.dst_w, m_param.dst_h,
//         m_input_norm_device, m_param.dst_w, m_param.dst_h, m_param);
//     std::cout << "normDevice completed." << std::endl;

//     std::cout << "Calling hwc2chwDevice..." << std::endl;
//     hwc2chwDevice(m_param.batch_size, m_input_norm_device, m_param.dst_w, m_param.dst_h,
//         m_input_hwc_device, m_param.dst_w, m_param.dst_h);
//     std::cout << "hwc2chwDevice completed." << std::endl;

//     std::cout << "Exiting preprocess function." << std::endl;
// }
...
YOLOv8Seg::DetectionResult YOLOv8Seg::showAndSave(const std::vector<std::string>& classNames, const int& cvDelayTime, std::vector<cv::Mat>& imgsBatch)
{
    ...
    for (size_t bi = 0; bi < imgsBatch.size(); bi++)
    {
        ...
        for (size_t i = 0; i < num_boxes; i++)
        {
            ...
            if (ptr[6])
            {
                ...
                cv::String det_info = m_param.class_names[label] + " " + cv::format("%.4f", ptr[4]);//类别+置信度
                ...
                // Create a new DetectedObject and fill it with the current object's data.
                DetectedObject obj;
                obj.bboxPoints.push_back(cv::Point(x_lt_src, y_lt_src));
                obj.bboxPoints.push_back(cv::Point(x_lt_src + det_info.size() * m_param.char_width, y_lt_src));
                obj.bboxPoints.push_back(cv::Point(x_lt_src + det_info.size() * m_param.char_width, y_lt_src - m_param.det_info_render_width));
                obj.bboxPoints.push_back(cv::Point(x_lt_src, y_lt_src - m_param.det_info_render_width));

                obj.masksInstance = mask_instance;

                obj.label = label;
                obj.confidence = ptr[4];

                // Add the object to the DetectionResult.
                result.objects.push_back(obj);
            }   
        }
        imgsBatch[bi] = imgsBatch[bi] + m_img_canvas;
        if (m_param.is_show)
        {
