...
//[]没清内存
void Map::AddObjectCenter(Eigen::Vector3f Object_Cts)
{
    unique_lock<mutex> lock(mMutexMap);
    ObjectCenters.push_back(Object_Cts);
    // cout<<"atlas centers is"<<*Object_Cts<<endl;
}

void Map::AddObject(Object newobjects, bool WhetherFirst)
{
    unique_lock<mutex> lock(mMutexMap);
    if(WhetherFirst)
    {
        ...
        {
        MapObjects->Objects.push_back(newobjects.Objects[i]);
        // cout<<"addobject frameid: "<<newobjects.Objects[i].Obj_Fms.back()<<" "<<newobjects.Objects[i].Obj_Fms.size()<<endl;
        MapObjects->Objects[i].mnId = i+1;
            for(auto mp : MapObjects->Objects[i].Obj_MPs)
            mp->ObjectId = MapObjects->Objects[i].mnId;
        }
    }
    else
    {
        int lastId = MapObjects->Objects.back().mnId;
        for(int i = 0; i < newobjects.Objects.size(); i++)
        {
        MapObjects->Objects.push_back(newobjects.Objects[i]);
        MapObjects->Objects.back().mnId = lastId+i+1;
            for(auto mp : MapObjects->Objects[i].Obj_MPs)
            mp->ObjectId = MapObjects->Objects[i].mnId;
        }
    }
...
void Map::AddObjectForTest(Object newobjects, bool WhetherFirst)
{
    unique_lock<mutex> lock(mMutexMap);
    if(WhetherFirst)
    {
        for(int i = 0; i < newobjects.Objects.size(); i++)
        {
        TestObjects->Objects.push_back(newobjects.Objects[i]);
        // cout<<"addobject frameid: "<<newobjects.Objects[i].Obj_Fms.back()<<" "<<newobjects.Objects[i].Obj_Fms.size()<<endl;
        TestObjects->Objects.back().mnId = i+1;
            for(auto mp : TestObjects->Objects[i].Obj_MPs)
            mp->ObjectId = TestObjects->Objects[i].mnId;
        }
    }
    else
    {
        int lastId = TestObjects->Objects.back().mnId;
        for(int i = 0; i < newobjects.Objects.size(); i++)
        {
        TestObjects->Objects.push_back(newobjects.Objects[i]);
        TestObjects->Objects.back().mnId = lastId+i+1;
            for(auto mp : TestObjects->Objects[i].Obj_MPs)
            mp->ObjectId = TestObjects->Objects[i].mnId;
        }
    }
...
void Map::ApplyScaledRotation(const Sophus::SE3f &T, const float s, const bool bScaledVel)
{
    ...
    //TODO []更新物体质心坐标
    // for (vector<Eigen::Vector3f>::iterator sit = ObjectCenters.begin(); sit != ObjectCenters.end(); sit++)
    for (int sit = 0; sit < ObjectCenters.size(); sit++)
    {
        // 更新每一个质心在世界坐标系下的坐标
        ObjectCenters[sit] = s * Ryw * ObjectCenters[sit] + tyw;
    }
