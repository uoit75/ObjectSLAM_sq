    ...
    mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false), mbShutDown(false)
{
    ...
    mpFrameDrawer = new FrameDrawer(mpAtlas, settings_);
...
//[superquadric]
Sophus::SE3f System::TrackRGBD_yolo(const cv::Mat &im, const cv::Mat &depthmap, const double &timestamp, const vector<DetectedObject>& detected_objects, const vector<IMU::Point>& vImuMeas, string filename)
{
    ...
    Sophus::SE3f Tcw = mpTracker->GrabImageRGBD_yolo(imToFeed, imDepthToFeed, timestamp, detected_objects, filename);
...
//[superquadric]
Sophus::SE3f System::TrackMonocular_yolo(const cv::Mat &im, const double &timestamp, const vector<TextLine>& lines, const vector<IMU::Point>& vImuMeas, string filename)
{
    ...
    Sophus::SE3f Tcw = mpTracker->GrabImageMonocular_yolo(imToFeed,timestamp,lines,filename);
...
bool System::IsBA2Finished(){
    return mpAtlas->GetCurrentMap()->GetIniertialBA2();
}

Eigen::Vector3f System::GetVel()
{
...
//[superquadric]
//for analyse
void System::SaveMappointPose(const string &filename)
{
    cout << endl << "Saving Mappoint pose to " << filename << " ..." << endl;

    Object AllObject = *(mpAtlas->GetCurrentMap()->MapObjects);
...
void System::SaveMappointDistance(const string &filename)
{
    cout << endl << "Saving Mappoint pose to " << filename << " ..." << endl;
    ...
    for(size_t i = 0; i < AllObject.Objects.size(); i++)
    { 
        Eigen::Vector3f FramePose = AllObject.Objects[i].FramePose;
        f <<"Frame ID: "<< AllObject.Objects[i].FrameId <<" Frame Pose: " <<FramePose(0)<<" "<<FramePose(1)<<" "<<FramePose(2)<< endl;
        f <<"ObjectId: "<<AllObject.Objects[i].mnId<<" ObjectClass: "<<AllObject.Objects[i].cid<<" ObjectCenter: "<<AllObject.Objects[i].Obj_Cen(0)<< " "<<AllObject.Objects[i].Obj_Cen(1)<< " "<<AllObject.Objects[i].Obj_Cen(2)<<endl;
        // f <<"ObjectId: "<<AllObject.Objects[i].mnId<<" ObjectCenter: "<<AllObject.Objects[i].Obj_Cen(0)<< " "<<AllObject.Objects[i].Obj_Cen(1)<< " "<<AllObject.Objects[i].Obj_Cen(2)<<endl;
        for(size_t j = 0; j < AllObject.Objects[i].Obj_MPs.size(); j++)
        {
        Eigen::Vector3f t = AllObject.Objects[i].Obj_MPs[j]->GetWorldPos();
