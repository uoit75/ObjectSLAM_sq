...
void Preintegrated::IntegrateNewMeasurement(const Eigen::Vector3f &acceleration, const Eigen::Vector3f &angVel, const float &dt)
{
   ...
   // 小量delta初始为0，更新后通常也为0，故省略了小量的更新
    ...
    C.block<9, 9>(0, 0) = A * C.block<9, 9>(0, 0) * A.transpose() + B * Nga * B.transpose();  // B矩阵为9*6矩阵 Nga 6*6对角矩阵，3个陀螺仪噪声
                                                                                                //的平方，3个加速度计噪声的平方
    ...
    JRg = dRi.deltaR.transpose() * JRg - dRi.rightJ * dt;//15-60
...
Eigen::Matrix3f Preintegrated::GetDeltaRotation(const Bias &b_)
{
    ...
    return NormalizeRotation(dR * Sophus::SO3f::exp(JRg * dbg).matrix());//15-56
}
