...
void Viewer::Run()
{
    ...
    // //[]所有的对象都放在这里
    // Object_Map* OBJECT_MAP = new Object_Map();

    mbFinished = false;
    mbStopped = false;

    pangolin::CreateWindowAndBind("Object Semantic Map",1024,768);
    ...
    pangolin::Var<bool> menuShowCamera("menu.Show Camera",false,true);
    ...
    pangolin::Var<bool> menuStepByStep("menu.Step By Step",false,true);  // false, true
    pangolin::Var<bool> menuStep("menu.Step",false,false);
    // pangolin::Var<int> menuInputObjectId("menu.Input ObjectId", 1, 1, 200); // 创建一个整数型的变量，初始值为42，范围在0到100之间
    pangolin::Var<bool> menuShowObjects("menu.Show Objects",true,true);
    pangolin::Var<bool> menuShowObjectsPoints("menu.Show ObjectsPoints",true,true);
    ...
    while(1)
    {
        ...
        if(menuStepByStep && !bStepByStep)
        // if(menuStepByStep && !bStepByStep && mpTracker->GetLastKeyFrame()->mnId > 50)
        {
            //cout << "Viewer: step by step" << endl;
            mpTracker->SetStepByStep(true);
            bStepByStep = true;
        ...
        if(menuShowPoints)
        {   
            mpMapDrawer->DrawMapPoints_background();
            // mpMapDrawer->DrawMapPoints_ObjectCenters();
            // // mpMapDrawer->DrawObjectQuadrics();
            // mpMapDrawer->DrawObjectSuperQuadrics();          
        }
        if(menuShowCamera)
        {   
            // glClearColor(1.0f,1.0f,1.0f,1.0f);
        ...
        // if(menuInputObjectId != 0)
        // {   
        //     // mpMapDrawer->DrawObjectQuadrics();
        //     mpMapDrawer->DrawAnObjectSuperQuadrics(menuInputObjectId);          
        // }
        if(menuShowObjectsPoints)
        {   
            mpMapDrawer->DrawMapPoints_objects();  
            mpMapDrawer->DrawMapPoints_ObjectCenters();
        }

        cv::namedWindow("Input images");
        cv::moveWindow("Input images", 1200, 0);
        cv::namedWindow("Instant Segmentation");
        cv::moveWindow("Instant Segmentation", 1200, 540);
        
        // cv::moveWindow("Object Semantic Map", 1200, 0);
        ...
        //[]
        // YOLO trackedImageYolo = mpTracker->GetImageYolo();
        // vector<cv::Vec4f> lines = mpTracker->GetLines();
        // cv::Mat im = mpFrameDrawer->DrawFrame_yolo_lines(trackedImageScale, trackedImageYolo, lines);
        // cv::Mat im = mpFrameDrawer->DrawFrame_yolo(trackedImageScale, trackedImageYolo);
 
        if(both){
            cv::Mat imRight = mpFrameDrawer->DrawRightFrame(trackedImageScale);
            cv::hconcat(im,imRight,toShow1);
        }
        else{
            toShow1 = im;
        }
        ...
        if(mImageViewerScale != 1.f)
        {
            int width = toShow1.cols * mImageViewerScale;
            int height = toShow1.rows * mImageViewerScale;
            cv::resize(toShow1, toShow1, cv::Size(width, height));
        }

        cv::imshow("Input images",toShow1);
        ...
        // vector<cv::Vec4f> lines = mpTracker->GetLines();
        // cv::Mat im = mpFrameDrawer->DrawFrame_yolo_lines(trackedImageScale, trackedImageYolo, lines);
        cv::Mat im2 = mpFrameDrawer->DrawFrame_yolo(trackedImageScale, trackedImageYolo);
        if(both){
            cv::Mat imRight = mpFrameDrawer->DrawRightFrame(trackedImageScale);
            cv::hconcat(im2,imRight,toShow2);
        }
        else{
            toShow2 = im2;
        }
        ...
        if(mImageViewerScale != 1.f)
        {
            int width = toShow2.cols * mImageViewerScale;
            int height = toShow2.rows * mImageViewerScale;
            cv::resize(toShow2, toShow2, cv::Size(width, height));
        }

        cv::imshow("Instant Segmentation",toShow2);
        ...
        if(menuReset)
        {
            ...
            // menuInputObjectId = true;
            menuShowObjectsPoints = true;
            menuLocalizationMode = false;
            if(bLocalizationMode)
                mpSystem->DeactivateLocalizationMode();
            bLocalizationMode = false;
