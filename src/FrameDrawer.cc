...
cv::Mat FrameDrawer::DrawFrame_yolo_lines(float imageScale, YOLO trackedImageYolo, vector<cv::Vec4f> lines)
{
    ...
    else if(state==Tracking::OK) //TRACKING
    {
        ...
        {   
            ...
            cv::line(im, Obj2Dcenter, imgXAxis, cv::Scalar(0, 0, 255), 3); 
            cv::line(im, Obj2Dcenter, imgYAxis, cv::Scalar(0, 255, 0), 3); 
            cv::line(im, Obj2Dcenter, imgZAxis, cv::Scalar(255, 0, 0), 3); 
            }
