...
void parseLine(TextLine l, string& cla, cv::Rect& box, string& mask, float& cT) ;
std::vector<cv::Point> NormalizationStringToPoint(const std::string& input, int Camerawidth, int Cameraheight);
 ...
 * Step 3 ：跟踪
 */
Sophus::SE3f Tracking::GrabImageMonocular_yolo(const cv::Mat &im, const double &timestamp,const vector<TextLine>& lines, string filename)
{

    //[]test
    // std::cout <<"tracking:"<< lines[0].text << std::endl;
    std::cout <<"tracking a Frame------------------------------------------------------------------------"<<endl;
    ...
    long unsigned int fidVec;
    vector<string> clsVec;
    vector<cv::Rect> bxsVec;
    vector<string> mksVec;
    vector<float> ctVec;
    ...
    fidVec = mCurrentFrame.mnId;
    for (const auto& line : lines) 
    {
        parseLine(line, cla, box, mask, cT);
        clsVec.push_back(cla);
        bxsVec.push_back(box);
        mksVec.push_back(mask);
        ctVec.push_back(cT);
        maskpoints.push_back(NormalizationStringToPoint(mask, Camerawidth, Cameraheight));
    }
    ...
    // cout<<"img matshow:"<<mImGray<<endl;
    // cout<<"lines size:"<<lines.size()<<endl;
    // cout<<"tracking clsVec:"<<clsVec.size()<<endl;
    YOLO mCurrentYolo(fidVec, clsVec, bxsVec, maskpoints, ctVec);
    // mCurrentYolo.printYOLO();

    mImageYolo = mCurrentYolo;
    ...
    // //[]初始化对象，这里是每一帧都初始化一个对象
    // Object CurrentObjects;
    // CurrentObjects = Object::InitialObject(mCurrentFrame, mImageYolo);

    // mObject = &CurrentObjects;

    // cout<<"now is altas turn"<<endl;
    // if(mState == OK)
    // {
    //     for(int i = 0; i < CurrentObjects.Objects.size(); i++)
    //     {
    //         mpAtlas->GetCurrentMap()->AddObjectCenter(CurrentObjects.Objects[i].Obj_Cen);
    //     }
    // }
...
void Tracking::PreintegrateIMU()
{
    ...
    for(int i=0; i<n; i++)
    {
        ...
        if((i==0) && (i<(n-1)))
        {
            ...
            // 有了这个改变量将其加到a0上之后就可以表示上一帧时的加速度了。
            // 其中a0 - (a1-a0)*(tini/tab) 为上一帧时刻的加速度再加上a1 之后除以2就为这段时间的加速度平均值
            // 其中tstep表示a1到上一帧的时间间隔，a0 - (a1-a0)*(tini/tab)这个式子中tini可以是正也可以是负表示时间上的先后，
            //(a1-a0)也是一样，多种情况下这个式子依然成立
    ...
    else
    {
        ...
        if(bOK || mState==RECENTLY_LOST)
        {
            ...
            {    
                ...
                // // cv::Mat grayImage;
                // // cv::cvtColor(mImGray, grayImage, cv::COLOR_BGR2GRAY);
                // vector<cv::Vec4f> lines_fld;
                // fld->detect(mImGray, lines_fld);
                // line_det = lines_fld;


                // cout<<"lines in tracking line: "<<endl;
                // for(auto line : mCurrentFrame.lines_det)
                // {
                //     cout<<line<<endl;
                // }

                // //初始化物体偏航
                // for(int l = 0; l < CurrentObjects.Objects.size(); l++)
                // {
                //     Object::ComputeRotation(CurrentObjects.Objects[l], mCurrentFrame, lines_fld);

                //     cout<<"object "<<l<<" :"<<CurrentObjects.Objects[l].rotationY<<"------------------------------"<<endl;
                // }
                ...
                if(mState == OK)
                {
                    ...
                    //如果没有物体的话
                    if(mpAtlas->GetCurrentMap()->MapObjects->Objects.empty())
                    {
                        mpAtlas->GetCurrentMap()->AddObject(CurrentObjects, true);
                        // mpAtlas->GetCurrentMap()->AddObjectForTest(CurrentObjects, true);//test
                    }
                        // mpAtlas->GetCurrentMap()->AddObjectForTest(CurrentObjects, true);  //test                    


                    //计算物体质心
                    // mpAtlas->GetCurrentMap()->MapObjects->ComputeObjectCenter();

                    // //剔除外点
                    // // Object::RemoveOutlier(*(mpAtlas->GetCurrentMap()->MapObjects));
                    // Object::RemoveOutlierXYZ(*(mpAtlas->GetCurrentMap()->MapObjects));
                    // Object::RemoveOutlierD(*(mpAtlas->GetCurrentMap()->MapObjects));

                    //计算物体质心
                    mpAtlas->GetCurrentMap()->MapObjects->ComputeObjectCenter();
                    // for(auto obj:mpAtlas->GetCurrentMap()->MapObjects->Objects)
                    // cout<<"OBJ TWQ: "<<obj.Twq<<endl;

                    //物体关联
                    Object newobjects; 
                    Object::KMassociation(CurrentObjects, *(mpAtlas->GetCurrentMap()->MapObjects), newobjects);                                     
                    // Object::Association(CurrentObjects, *(mpAtlas->GetCurrentMap()->MapObjects), newobjects);
                    cout<<"newbject size:"<<newobjects.Objects.size()<<endl;
                    


                    // //计算物体质心和均值方差
                    // newobjects.ComputeObjectCenter();
                    // for(int k = 0; k < newobjects.Objects.size(); k++)
                    // Object::CommputeDistanceSD(newobjects.Objects[k]);

                    if(!newobjects.Objects.empty())
                    {
                        // Object::IndependentTtest(*(mpAtlas->GetCurrentMap()->MapObjects), newobjects);
                        // if(!newobjects.Objects.empty())
                        mpAtlas->GetCurrentMap()->AddObject(newobjects, false);                      
                    }
                    mpAtlas->GetCurrentMap()->MapObjects->ComputeObjectCenter();
                    ...
                    {   
                        mpAtlas->GetCurrentMap()->MapObjects->Objects[k].WhetherInFrame = false;

                        int MPsize = mpAtlas->GetCurrentMap()->MapObjects->Objects[k].Obj_MPs.size();
                        int OBJsize = mpAtlas->GetCurrentMap()->MapObjects->Objects.size();
                        int FMsize = mpAtlas->GetCurrentMap()->MapObjects->Objects[k].Obj_Fms.size();
                        int LastFM = mpAtlas->GetCurrentMap()->MapObjects->Objects[k].Obj_Fms.back();
                        // if (mpAtlas->GetCurrentMap()->MapObjects->Objects[k].Obj_MPs.size() <= 30 && k < mpAtlas->GetCurrentMap()->MapObjects->Objects.size()-5)
                        if ((MPsize <= 10) || //地图点少于10的
                        (FMsize < 3 && ((LastFM + 100) < mCurrentFrame.mnId))) //少于3帧观察到的
                        // if ((MPsize <= 10) )//地图点少于10的 
                        {
                            indicesToDelete.push_back(k);
                            for(int l = 0; l < MPsize; l++)
                            {
                                mpAtlas->GetCurrentMap()->MapObjects->Objects[k].Obj_MPs[l]->whetherInObject = false;
                            }
                        }
                    }
                    // 根据记录的位置删除元素（从后往前删除）
                    ...
                    {   
                        ...
                        mpAtlas->GetCurrentMap()->MapObjects->Objects.erase(mpAtlas->GetCurrentMap()->MapObjects->Objects.begin() + index);
                    }
                    ...
                    {
                        //删除不在物体内的地图点 很卡
                        for(int l = 0; l < mpAtlas->GetReferenceMapPoints().size(); l++)
                        {
                            bool InObject = false;
                            for (int m = 0; m < mpAtlas->GetCurrentMap()->MapObjects->Objects.size(); m++)
                            {
                                // Object::Object_3D CurrentObject = mpAtlas->GetCurrentMap()->MapObjects->Objects[m];
                                // cv::Mat Twq = CurrentObject.Twq;

                                // cv::Mat Twq_inv = Twq.inv();
                                // float e1 = CurrentObject.e1;
                                // float e2 = CurrentObject.e2;


                                // Eigen::Vector3f pos = mpAtlas->GetReferenceMapPoints()[l]->GetWorldPos();
                                // double euation1 = (Twq_inv.at<float>(0,0)*pos(0) + Twq_inv.at<float>(0,1)*pos(1) + Twq_inv.at<float>(0,2)*pos(2) + Twq_inv.at<float>(0,3)) / CurrentObject.length;
                                // double euation2 = (Twq_inv.at<float>(1,0)*pos(0) + Twq_inv.at<float>(1,1)*pos(1) + Twq_inv.at<float>(1,2)*pos(2) + Twq_inv.at<float>(1,3)) / CurrentObject.width;
                                // double euation3 = (Twq_inv.at<float>(2,0)*pos(0) + Twq_inv.at<float>(2,1)*pos(1) + Twq_inv.at<float>(2,2)*pos(2) + Twq_inv.at<float>(2,3)) / CurrentObject.height;
                                // double F = pow((pow(euation1, 2/e2) + pow(euation2, 2/e2)), e2/e1) + pow(euation3, 2/e1);
                                // if(F<=1)InObject = true;
                            

                                if(mpAtlas->GetReferenceMapPoints()[l]->ObjectId == mpAtlas->GetCurrentMap()->MapObjects->Objects[m].mnId)
                                {
                                    InObject = true;
                                }
                            }
                            if(InObject == false)
                            {
                            mpAtlas->GetReferenceMapPoints()[l]->whetherInObject = false;
                            // cout<<"delete mappoints-----------"<<endl;
                            }
                        }
...
void Tracking::CreateInitialMapMonocular()
{
    ...
    //[]物体中心点尺度归一化  没用
    // mpAtlas->GetCurrentMa
    for (int sit = 0; sit < mpAtlas->GetCurrentMap()->ObjectCenters.size(); sit++)
    {
        // 更新每一个质心在世界坐标系下的坐标
        mpAtlas->GetCurrentMap()->ObjectCenters[sit] = mpAtlas->GetCurrentMap()->ObjectCenters[sit] * invMedianDepth;
    }
...
void Tracking::UpdateLocalKeyFrames()
{
            ...
            if(pMP)
            {
                if(!pMP->isBad())
                {
                    ...
                    for(map<KeyFrame*,tuple<int,int>>::const_iterator it=observations.begin(), itend=observations.end(); it!=itend; it++)
	                    ...
	                    // 所以最后keyframeCounter 第一个参数表示某个关键帧，第2个参数表示该关键帧看到了
                        // 多少当前帧(mCurrentFrame)的地图点，也就是共视程度
    ...
    for(vector<KeyFrame*>::const_iterator itKF=mvpLocalKeyFrames.begin(), itEndKF=mvpLocalKeyFrames.end(); itKF!=itEndKF; itKF++)
    {
        ...
        // 类型1:一级共视关键帧的共视（前10个）关键帧，称为二级共视关键帧（将邻居的邻居拉拢入伙）
        ...
        // 类型2:将一级共视关键帧的子关键帧作为局部关键帧（将邻居的孩子们拉拢入伙）
...
//[]
YOLO Tracking::GetImageYolo()
{
    return mImageYolo;
}

vector<cv::Vec4f> Tracking::GetLines()
{
    return line_det;
}

//[]没用
Object* Tracking::GetObjects()
{
    return mObject;
}
...
void parseLine(TextLine l, string& cla, cv::Rect& box, string& mask, float& cT) 
{
    ...
    while (ss >> token) 
    {
        count++;
        // strcount += token.size();
        // cout<<"strcount is:"<<strcount<<endl;
        if (count == 1) {
            cla = token;
            ...
            if (count == 2) {
                ...
                // cout<<"l.text.length() is:"<<l.text.length()<<endl;
                // cout<<"count is:"<<count<<endl;
                // cout<<"mask is:"<<token<<endl;               
                // if(count == l.text.length()-2)
                // cout<<"now mask is:"<<stof(token)<<endl;
        } 
    }
        // 查找mask最后一个空格的位置
        size_t lastSpacePos = mask.find_last_of(' ');
        if (lastSpacePos != std::string::npos) {
            // 删除最后一个空格后的所有内容
            mask = mask.substr(0, lastSpacePos);
        }
        // mask.pop_back();
        cT = stof(token);     
}
...
std::vector<cv::Point> NormalizationStringToPoint(const std::string& input, int Camerawidth, int Cameraheight) {
    ...
    while (iss >> x >> y) {
        points.emplace_back(round(x*Camerawidth), round(y*Cameraheight));
    }
