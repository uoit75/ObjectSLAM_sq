...
// 比较函数，用于根据 map 的值对键值对进行排序
static bool compare(const std::pair<KeyFrame*, int>& a, const std::pair<KeyFrame*, int>& b) {
...
//[superquadric]
//Retrieve images with YOLO information.
Sophus::SE3f Tracking::GrabImageRGBD_yolo(const cv::Mat &imRGB,const cv::Mat &imD, const double &timestamp, const vector<DetectedObject>& detected_objects, string filename)
{
    ...
    mImGray = imRGB;
    cv::Mat imDepth = imD;
    // Step 1 ：Convert the color image to a grayscale image.
    if(mImGray.channels()==3)
    {
        if(mbRGB)
            cvtColor(mImGray,mImGray,cv::COLOR_RGB2GRAY);
    ...
    // for (const auto& detected_object : detected_objects) 
    // {
    //     clsVec.push_back(std::to_string(detected_object.label));
    //     bxsVec.push_back(detected_object.bbox_points);
    //     // mksVec.push_back(mask);
    //     ctVec.push_back(detected_object.confidence);
    //     maskpoints.push_back(detected_object.masks_instance);
    // }
    ...
    //     // 打印每个对象的属性
    //     std::cout << "Label: " << detected_object.label << std::endl;
    //     std::cout << "Bounding Box Points: ";
    //     // 打印边界框点的坐标和尺寸
    //     const cv::Rect& bbox = detected_object.bbox_points;
    ...
    //     std::cout << "Confidence: " << detected_object.confidence << std::endl;

    //     std::cout << "Masks Instance Points: ";
    //     for (const auto& mask_point : detected_object.masks_instance) {
    //         std::cout << "(" << mask_point.x << ", " << mask_point.y << ") ";
    //     }
    //     std::cout << std::endl;
    ...
    if(detected_objects.size()>0)
    {
        ...
        // std::ofstream outfile(file_name);  // 使用唯一文件名创建输出文件流对象
        ...
        for (const auto& detected_object : detected_objects) 
        {
            ...
            // // 写入每个对象的属性到文件中
            // outfile << "Label: " << detected_object.label << std::endl;

            // // 写入边界框点的坐标和尺寸
            // const cv::Rect& bbox = detected_object.bbox_points;
            // outfile << "Bounding Box: [x: " << bbox.x 
            //         << ", y: " << bbox.y 
            //         << ", width: " << bbox.width 
            //         << ", height: " << bbox.height 
            //         << "]" << std::endl;
            ...
            // // 写入置信度
            // outfile << "Confidence: " << detected_object.confidence << std::endl;

            // // 写入掩码实例的点
            // outfile << "Masks Instance Points: ";
            // for (const auto& mask_point : detected_object.masks_instance) {
            //     outfile << "(" << mask_point.x << ", " << mask_point.y << ") ";
            // }
            // outfile << std::endl;
        ...
        // outfile.close();  // 关闭文件流
        // std::cout << "Object detection information saved to " << file_name << std::endl;
...
Sophus::SE3f Tracking::GrabImageMonocular_yolo(const cv::Mat &im, const double &timestamp,const vector<TextLine>& lines, string filename)
{
    ...
    // Step 2 ：Construct the Frame class.
...
//[superquadric]
//The main tracking thread for object-level SLAM.
void Tracking::Track_yolo(bool whether_obj)
{
    ...
    else
    {
        ...
        if(bOK || mState==RECENTLY_LOST)
        {
            ...
            {    
                ...
                if(mState == OK && whether_obj == true)
                {
                    //[superquadric]-----------------------------------------------------------------------------------------------------
                    //Tracking successful, initialize object, creating once per keyframe.
                    Object CurrentObjects;
                    CurrentObjects = Object::InitialObject(mCurrentFrame, mImageYolo);

                    //If there are no objects in the map.
                    ...
                    {   
                        ...
                        if ((MPsize <= 10) || //mappoints less than 10
                        (FMsize < 3 && ((LastFM + 100) < mCurrentFrame.mnId))) //frames less than 3 
                        {
                    ...
                    {
                        //Remove map points that are not inside the object.
            ...
            }//The object-level SLAM tracking thread ends here.----------------------------------------------------------------
...
void Tracking::UpdateLocalKeyFrames()
{
    ...
    std::vector<std::pair<KeyFrame*, int>> vec(keyframeCounter.begin(), keyframeCounter.end());

    // 根据 int 值进行排序
    std::sort(vec.begin(), vec.end(), compare);

    // 创建一个新的 map，用于存储排序后的结果
    std::map<KeyFrame*, int> sortedKfCounter;
    for (const auto& pair : vec) {
        sortedKfCounter[pair.first] = pair.second;
    }
    ...
    for(map<KeyFrame*,int>::const_iterator it=sortedKfCounter.begin(), itEnd=sortedKfCounter.end(); it!=itEnd; it++)
    {
...
//Read the information from YOLO line.
void parseLine(TextLine l, string& cla, cv::Rect& box, string& mask, float& cT) 
{
