            ...
            if(!CheckNewKeyFrames() && !stopRequested())
            {
                ...
                if(mpAtlas->KeyFramesInMap()>2)
                {
                    ...
                    else
                    {
                        ...
                        //[]
                        Optimizer::ObjectOptimize(mpAtlas->GetCurrentMap(), mpAtlas->GetCurrentMap()->MapObjects);
                        // Optimizer::ObjectOptimizeE12(mpAtlas->GetCurrentMap(), mpAtlas->GetCurrentMap()->MapObjects);
                        // Optimizer::ObjectOptimizeSimulatedAnnealing(mpAtlas->GetCurrentMap(), mpAtlas->GetCurrentMap()->MapObjects);
                        
                        // if(mpAtlas->GetCurrentMap()->MapObjects->Objects.size() > 0)
                        // {
                        //     count_F++;
                        //     Optimizer::ObjectOptimizeForAnalyse(mpAtlas->GetCurrentMap(), mpAtlas->GetCurrentMap()->MapObjects);
 ...
 * @param priorA 加速度计偏置的信息矩阵系数 信息矩阵值越大，可信度越高
...
void LocalMapping::InitializeIMU(float priorG, float priorA, bool bFIBA)
{
    ...
    if (!mpCurrentKeyFrame->GetMap()->isImuInitialized())
    {
        ...
        {
            if (!(*itKF)->mpImuPreintegrated)//当前帧到上一帧的预积分不存在则跳过
                continue;
            if (!(*itKF)->mPrevKF)//当前帧的上一帧不存在则跳过
                continue;
