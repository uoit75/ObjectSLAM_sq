...
int Object::DBSCAN::run()
{
    ...
    for (size_t i = 0; i < m_points.size(); ++i)
    {
        ...
        {
            if (expandCluster(i, clusterID) != DBSCAN_FAILURE)
            {
                clusterID += 1;
            }
        }
...
int Object::DBSCAN::expandCluster(int pointIndex, int clusterID)
{    
    vector<int> clusterSeeds = calculateCluster(m_points[pointIndex]);

    if (clusterSeeds.size() < m_minPoints)
    {
        clusterIDs[pointIndex] = NOISE;
        return DBSCAN_FAILURE;
    }
    else
    {
        for (int seedIndex : clusterSeeds)
        {
            clusterIDs[seedIndex] = clusterID;
        }

        for (size_t i = 0; i < clusterSeeds.size(); ++i)
        {
            int neighborIndex = clusterSeeds[i];
            vector<int> clusterNeighbors = calculateCluster(m_points[neighborIndex]);

            if (clusterNeighbors.size() >= m_minPoints)
            {
                for (int neighbor : clusterNeighbors)
                {
                    if (clusterIDs[neighbor] == UNCLASSIFIED || clusterIDs[neighbor] == NOISE)
                    {
                        if (clusterIDs[neighbor] == UNCLASSIFIED)
                        {
...
vector<int> Object::DBSCAN::calculateCluster(cv::Point3f point)
{
...
double Object::DBSCAN::calculateDistance(const cv::Point3f& pointCore, const cv::Point3f& pointTarget)
{
    return pow(pointCore.x - pointTarget.x, 2) + pow(pointCore.y - pointTarget.y, 2) + pow(pointCore.z - pointTarget.z, 2);
}

// RemoveOutlierDBSCAN function implementation
void Object::RemoveOutlierDBSCAN(Object_3D &Obj, float MINIMUM_POINTS, float EPSILON)
{
    // Prepare points from Obj.Obj_MPs for DBSCAN
    std::vector<cv::Point3f> points;
    for (auto &mp : Obj.Obj_MPs) {
        if (mp->isBad()) continue;

        Eigen::Vector3f pos = mp->GetWorldPos();
        points.push_back(cv::Point3f(pos.x(), pos.y(), pos.z()));
    }
    ...
    // Collect inliers based on DBSCAN clustering results
    std::vector<MapPoint*> inlierPoints;
    for (size_t i = 0; i < points.size(); ++i) {
        if (dbscan.clusterIDs[i] != NOISE) {
            inlierPoints.push_back(Obj.Obj_MPs[i]);
        } else {
            Obj.Obj_MPs[i]->whetherInObject = false;
        }
    }
    Obj.Obj_MPs = inlierPoints;
}
/////////////DBSCAN
